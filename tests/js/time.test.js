...
test('getUpcoming: горизонт отсекает, прошедшие не попадают', () => {
  const near = ev('2026-07-11T12:30', null, { title: 'близко' });
  const far = ev('2026-07-11T14:01', null, { title: 'далеко' });
  const past = ev('2026-07-11T11:00', null, { title: 'прошло' });
  const got = core.getUpcoming([far, past, near], MSK('2026-07-11T12:00'), 60);
  assert.deepEqual(got.map(e => e.title), ['близко']);
});
